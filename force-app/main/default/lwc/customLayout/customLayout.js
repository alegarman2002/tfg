...
export default class CustomLayout extends LightningElement {
  ...
  async renderedCallback() {
    try {
      this.init();
      var control = 0;
      await compareEventsId({ id: this.eventid }).then((atribute) => {
        control = atribute;
      });
      console.log(control);
      if (control == 1) {
        var alerta = this.template.querySelector(
          '[data-id="controlRegistration"]'
        );
  ...
  async init() {
    ...
    let myData = null;
    await getEventInfo({ id: tracker }).then((atribute) => {
      myData = atribute;
    });

    const owner = myData[0];
    let ownerData = null;
    await getUserInformation({ eventId: owner }).then((atribute) => {
      ownerData = atribute;
    });
    ...
    this.timeOfEvent =
      ...
      " - " +
      endDateTime.split(" ")[1].substr(0, 5);
    ...
    const newStart =
      `${newStartHour.toString().padStart(2, "0")}:` + startMinute;
  ...
  async registerForEvent() {
    ...
    await getUserLogInfo().then((atribute) => {
      listOfAttributes = atribute;
    });
